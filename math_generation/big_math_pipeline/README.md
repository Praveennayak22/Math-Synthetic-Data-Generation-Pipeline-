...
```python
{
    ...
    "Difficulty level": int,           # 0-4 complexity score
    "Number of words in system prompt": int,  # Word count metric
    "Number of words in question": int,      # Word count metric
    "Number of words in answer": int,        # Word count metric
    ...
    "Made by": str,                    # Generator model name
    "Reasoning/SFT": str,              # Format type for training
...
|-------|------|-------------|
| `SFT/Reasoning` | str | Type of dataset: "Reasoning" indicates reasoning traces are included |
| `System prompt` | str | System-level instructions for the model |
| `Instruction` | str | User instruction/context for the problem |
| `Question` | str | The actual math problem to solve |
| `<Think>` | str | Step-by-step reasoning/chain-of-thought generated by model |
| `Answer` | str | Final answer generated by model |
| `Difficulty level` | int | 0-4 scale: 0=Minimal, 1=Basic, 2=Intermediate, 3=Advanced, 4=Expert |
| `Number of words in system prompt` | int | Length metric for system prompt |
| `Number of words in question` | int | Length metric for problem statement |
| `Number of words in answer` | int | Length metric for answer |
| `Task` | str | Task classification (usually "QA" for question-answering) |
| `Domain` | str | Subject domain (math, algebra, geometry, calculus, etc.) |
| `Language` | str | Language code (e.g., "English") |
| `Source` | str | Original dataset source before generation/mutation |
| `Made by` | str | Generator/mutation model used |
| `Reasoning/SFT` | str | Format for fine-tuning ("Reasoning" = with thinking traces) |
| `Model` | str | Teacher/reasoning model used (e.g., "kimi-k2.5") |
| `sample_id` | str | Unique identifier for this sample |
| `verified` | bool | True if answer passed Math-Verify validation |
...
print('=== Question ===')
print(sample['Question'][:300] + '...')
print()
print('=== Reasoning (<Think> - first 300 chars) ===')
print(sample['<Think>'][:300] + '...')
print()
print('=== Answer ===')
print(sample['Answer'])
print()
...
print(f'Word count (question): {sample[\"Number of words in question\"]}')
print(f'Word count (answer): {sample[\"Number of words in answer\"]}')
"
```
